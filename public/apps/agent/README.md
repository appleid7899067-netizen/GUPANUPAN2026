...
- 🛠 **เอเจนต์จริง + 8 สกิลข้อมูลสด** (`skills.js` ไม่ต้องใช้คีย์):
  `weather` อากาศ · `crypto` ราคาเหรียญ · `fx` ค่าเงิน · `wiki` สาระ ·
  `hn_news` ข่าวเทค · `calc` คำนวณ · `datetime` วันเวลาไทย · `fetch_url` ดึงเว็บ —
  AI เรียกใช้เองได้หลายรอบ (โชว์ trace ให้ดู) หรือสั่งตรงด้วย `/weather ภูเก็ต`
  `/crypto btc` `/fx` `/wiki …` `/news` `/calc …` `/time` `/fetch …` `/help`
