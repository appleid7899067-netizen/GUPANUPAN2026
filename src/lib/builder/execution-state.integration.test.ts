...
const maybe = {
  skip: !testUrl
    ? "TEST_DATABASE_URL is required; these are real PostgreSQL integration tests"
    : false,
};
...
async function cleanup(id: string) {
  if (pool) await pool.query("DELETE FROM builder_execution_state WHERE id = $1", [id]);
}

test(
  "optimistic lock: two concurrent transitions with the same version allow exactly one",
  maybe,
  async () => {
    assert(api);
    const id = uid();
...
test(
  "FOR UPDATE blocks a second PostgreSQL connection until commit",
  maybe,
  async () => {
    assert(api);
    assert(pool);
    ...
    try {
      await a.query("BEGIN");
      await a.query(
        "SELECT id FROM builder_execution_state WHERE id = $1 FOR UPDATE",
        [id],
      );
      ...
      assert.ok(
        ...
        "second connection did not visibly wait for the lock",
      );
    } finally {
...
test(
  "idempotency claim is atomic: same key is accepted once",
  maybe,
  async () => {
    assert(api);
    const id = uid();
    ids.push(id);
    await api.createExecution({ id, goal: "idempotency" });

    const [a, b] = await Promise.all([
      api.claimIdempotency(id, "same-key", { winner: "a" }),
      api.claimIdempotency(id, "same-key", { winner: "b" }),
    ]);
...
test(
  "cancellation is visible to a transition after the cancellation transaction commits",
  maybe,
  async () => {
    assert(api);
    const id = uid();
    ids.push(id);
    await api.createExecution({ id, goal: "cancel" });
    await api.requestCancellation(id, "user cancelled");

    await assert.rejects(
      api.transitionExecution(id, "PLANNING", "must not run after cancel"),
      /cancellation requested/i,
    );

    const execution = await api.getExecution(id);
    assert.equal(execution.cancelRequested, true);
  },
);
