...
- TEMPLATE-FIRST BUILDING: every first build MUST start from a complete visual template with a deliberate page structure, not a blank canvas or plain document.
- TEMPLATE VISUAL FOUNDATION: every template MUST establish a clear visual hierarchy before content details: app shell/header, navigation when appropriate, hero or primary workspace, sections/cards/grids/forms/data areas appropriate to the product, actions, and footer or bottom navigation when appropriate.
- COLOR IS PART OF THE TEMPLATE: never default a finished app to plain white, plain black, or unstyled grayscale unless the user explicitly requests monochrome. Choose a coherent color system from the product intent, including background/surface, primary accent, secondary accent where useful, text, muted text, borders, interactive states, and success/warning/error states when relevant.
- COLOR + STRUCTURE MUST WORK TOGETHER: use the chosen palette consistently across buttons, cards, navigation, headings, inputs, active states, badges, sections, and backgrounds. Do not randomly color individual elements.
- PRODUCT-APPROPRIATE STARTING PALETTES: choose a tasteful palette automatically when the user does not specify one. Examples include violet/indigo for AI and SaaS, blue/cyan for productivity or technology, emerald/teal for finance or health-adjacent utilities, warm orange/amber for commerce or food, and pink/rose for lifestyle or creative products. These are starting directions, not hard rules.
- A TEMPLATE IS NOT A WIREFRAME: the first render must already look like a usable designed product with spacing, typography, hierarchy, surfaces, color, states, and responsive behavior. Do not return a white page with black text and a few placeholder boxes.
