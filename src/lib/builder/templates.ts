...
function makeCatalogTemplate(title: string, category: string, style: string, accent: string, features: string[]): ExampleApp {
  ...
  const appLike = /^(SaaS|Finance|Ecommerce|Internal Tools|Business|Productivity|AI Apps|Developer Tools|Education|Fitness|Real Estate|Community)$/i.test(category);
  const siteLike = /^(Websites|Food|Agency|Portfolio|Content|Travel)$/i.test(category);

  const body = appLike
    ? `
  <div class="app-shell">
    <aside class="sidebar">
      <div class="brand"><strong>${title}</strong><span class="status-dot"></span></div>
      <nav class="side-nav"><a class="active" href="#main">Overview</a><a href="#features">Workspace</a><a href="#items">Data</a><a href="#contact">Settings</a></nav>
      <div class="side-card"><span class="eyebrow">Starter</span><b>Ready to build</b><p>Ask Boss to connect real data, auth or APIs.</p></div>
    </aside>
    <main class="workspace" id="main">
      <header class="workspace-head"><div><span class="eyebrow">${category} · ${style}</span><h1>${title}</h1><p>Production-shaped workspace with a real information architecture, not a single long page.</p></div><div class="actions"><button id="primary">Create</button><button id="start" class="ghost">Share</button></div></header>
      <section class="kpis"><article class="metric"><span>Active</span><strong>24</strong><small>+12% this week</small></article><article class="metric"><span>Completed</span><strong>86%</strong><small>On track</small></article><article class="metric"><span>Pending</span><strong>08</strong><small>Needs attention</small></article></section>
      <section id="features" class="featuregrid">${featureHtml}</section>
      <section class="card work"><div class="section-head"><div><span class="eyebrow">Workspace</span><h2>Explore ${category.toLowerCase()}</h2></div><div class="toolbar"><input id="search" placeholder="Search..."><span id="count"></span></div></div><div id="items" class="itemgrid">${itemHtml}</div></section>
      <section id="contact" class="card contact"><div><span class="eyebrow">Next step</span><h2>Make it yours</h2><p>Connect the real backend, replace sample data and keep this structure.</p></div><button id="contactBtn">Continue</button></section>
    </main>
  </div>`
    : siteLike
    ? `
  <nav class="shell site-nav"><strong>${title}</strong><div class="navlinks"><a href="#features">Features</a><a href="#work">Work</a><a href="#contact">Contact</a><button id="start">Start</button></div></nav>
  <main id="main">
    <section class="shell hero hero-split"><div><span class="eyebrow">${category} · ${style}</span><h1>${title}</h1><p>A complete starter structure with a clear story, reusable sections, visual hierarchy and responsive behavior.</p><div class="actions"><button id="primary">Get started</button><button class="ghost" onclick="document.getElementById('work').scrollIntoView()">View work</button></div></div><div class="hero-art"><div class="hero-art-inner"><span>Preview</span><b>${category}</b></div></div></section>
    <section id="features" class="shell section"><div class="section-head"><div><span class="eyebrow">Structure</span><h2>Everything has a place</h2></div><p>Sections become components instead of one giant vertical block.</p></div><div class="featuregrid">${featureHtml}</div></section>
    <section id="work" class="shell section"><div class="section-head"><div><span class="eyebrow">Collection</span><h2>Explore the starter content</h2></div><div class="toolbar"><input id="search" placeholder="Search..."><span id="count"></span></div></div><div id="items" class="itemgrid">${itemHtml}</div></section>
    <section id="contact" class="shell card contact"><div><span class="eyebrow">Ready</span><h2>Turn the template into a real product.</h2><p>Swap the content, connect actions and let Boss extend the structure without flattening it.</p></div><button id="contactBtn">Continue</button></section>
  </main>`
    : `
  <nav class="shell site-nav"><strong>${title}</strong><div class="navlinks"><a href="#main">Home</a><a href="#features">Explore</a><a href="#contact">Contact</a><button id="start">Start</button></div></nav>
  <main id="main">
    <section class="shell hero"><span class="eyebrow">${category} · ${style}</span><h1>${title}</h1><p>Flexible catalog starter with filters, cards, actions and extension points already arranged.</p><div class="actions"><button id="primary">Try it</button><button class="ghost" onclick="document.getElementById('features').scrollIntoView()">Explore</button></div></section>
    <section id="features" class="shell bento">${featureHtml}</section>
    <section class="shell card work"><div class="section-head"><div><span class="eyebrow">Browse</span><h2>Starter collection</h2></div><div class="toolbar"><input id="search" placeholder="Search..."><span id="count"></span></div></div><div id="items" class="itemgrid">${itemHtml}</div></section>
    <section id="contact" class="shell card contact"><div><span class="eyebrow">Next step</span><h2>Make it yours</h2><p>Replace sample content with the real data model and connect the actions you need.</p></div><button id="contactBtn">Continue</button></section>
  </main>`;
  ...
  *{box-sizing:border-box}body{margin:0;background:${bg};color:${ink};font-family:ui-sans-serif,system-ui,sans-serif}a{color:inherit;text-decoration:none}.shell{max-width:1120px;margin:auto;padding:0 20px}
  ...
  .app-shell{min-height:100vh;display:grid;grid-template-columns:230px 1fr}.sidebar{position:sticky;top:0;height:100vh;padding:22px 16px;border-right:1px solid ${dark?"#30363d":"#ddd"};background:${dark?"rgba(10,14,18,.72)":"rgba(255,255,255,.5)"};display:flex;flex-direction:column;gap:28px}.brand{display:flex;align-items:center;gap:8px}.status-dot{width:7px;height:7px;border-radius:50%;background:${accent}}.side-nav{display:grid;gap:5px}.side-nav a{padding:10px 12px;border-radius:10px;color:${muted};font-size:14px}.side-nav a.active{background:${dark?"rgba(255,255,255,.08)":"#fff"};color:${ink}}.side-card{margin-top:auto;padding:14px;border:1px solid ${dark?"#30363d":"#ddd"};border-radius:14px;background:${dark?"rgba(255,255,255,.03)":"rgba(255,255,255,.7)"}}.side-card b{display:block;margin-top:6px}.side-card p{font-size:12px;color:${muted};line-height:1.5}.workspace{max-width:1280px;width:100%;padding:0 28px 70px}.workspace-head{display:flex;justify-content:space-between;gap:24px;align-items:end;padding:58px 0 28px}.workspace-head h1{margin:8px 0;font-size:clamp(38px,6vw,64px)}.workspace-head p{max-width:650px;color:${muted};line-height:1.55}.kpis{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px}.metric{padding:18px;border:1px solid ${dark?"#30363d":"#ddd"};border-radius:${radius};background:${dark?"rgba(255,255,255,.035)":"rgba(255,255,255,.65)"}}.metric span,.metric small{display:block;color:${muted};font-size:12px}.metric strong{display:block;font-size:30px;margin:8px 0}.section{padding-top:72px;padding-bottom:18px}.section-head{display:flex;justify-content:space-between;gap:20px;align-items:end;margin-bottom:16px}.section-head h2{margin:5px 0 0;font-size:24px}.section-head>p{max-width:420px;color:${muted};font-size:14px;line-height:1.5}.hero-split{display:grid;grid-template-columns:1.05fr .95fr;gap:40px;align-items:center}.hero-art{min-height:360px;border-radius:${radius};background:${accent};display:grid;place-items:center;overflow:hidden}.hero-art-inner{width:72%;aspect-ratio:4/3;border:1px solid rgba(255,255,255,.45);border-radius:${radius};padding:18px;display:flex;flex-direction:column;justify-content:space-between;color:${dark?"#111":"#fff"};background:rgba(255,255,255,.16)}.hero-art-inner b{font-size:34px;letter-spacing:-.04em}.bento{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;padding-bottom:24px}.bento .card:nth-child(1){grid-column:span 2;grid-row:span 2}.bento .card:nth-child(2){grid-column:span 2}.site-nav{height:72px}.contact{display:flex;justify-content:space-between;align-items:center;gap:20px;margin-top:28px;margin-bottom:70px}.contact h2{margin:6px 0;font-size:28px}.contact p{margin:0;color:${muted};max-width:620px}.contact button{white-space:nowrap}
