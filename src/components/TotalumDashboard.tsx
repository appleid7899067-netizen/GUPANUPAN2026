...
export default function DashboardPage() {
  ...
  // Hybrid: also checks Puter availability so dashboard doesn't show Totalum banner when Puter is ready.
  const [keyConfigured, setKeyConfigured] = useState<boolean | null>(null);
  const [configMode, setConfigMode] = useState<string | null>(null);
  useEffect(() => {
    api.get<{ configured: boolean; mode?: string; totalumConfigured?: boolean; puterEnabled?: boolean }>("/api/config").then((r) => {
      if (r.ok && r.data) {
        // New API returns mode; fallback to old boolean
        const mode = (r.data as any).mode || (r.data.configured ? "totalum" : "unconfigured");
        setConfigMode(mode);
        // In hybrid, Puter counts as configured (so no Totalum banner)
        const actuallyConfigured = mode !== "unconfigured";
        setKeyConfigured(actuallyConfigured);
        // Also check client-side Puter availability for instant UI feedback
        if (!actuallyConfigured && typeof window !== "undefined" && isPuterAvailable()) {
          setConfigMode("puter");
    ...
    // also watch for late Puter load
    const id = setInterval(() => {
      if (isPuterAvailable() && keyConfigured === false) {
        setConfigMode("puter");
        setKeyConfigured(true);
      }
    }, 1500);
  ...
  */
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok && res.data) {
        ...
        // If Totalum returns empty but Puter has legacy projects (from original Puter builder), merge them in hybrid mode
        if (list.length === 0 && typeof window !== "undefined" && isPuterAvailable()) {
          try {
            const puterProjects = await puterListProjects();
            if (puterProjects.length > 0) {
              // map to VcaasProjectSummary shape
              const mapped = puterProjects.map((p: any) => ({
        ...
        setProjects(list);
      } else if (typeof window !== "undefined" && isPuterAvailable()) {
        // Totalum error (e.g. no key) — fallback to Puter
        try {
          const puterProjects = await puterListProjects();
          const mapped = puterProjects.map((p: any) => ({
            projectId: p.projectId,
  ...
  // Hybrid: if in Puter mode or Totalum fails with auth, fall back to Puter storage.
  const createViaPuterFallback = async (id: string, prompt: string) => {
    try {
      const { createProject, saveProject } = await import("@/lib/builder");
      const proj = createProject(prompt);
      saveProject(proj);
      sessionStorage.setItem(`gupan:prompt:${proj.id}`, prompt);
      toast.info("เปิดโหมด frontend ในเครื่อง — กดส่งคำสั่งเพื่อเริ่มใช้ Puter AI");
      router.push(`/build/${proj.id}`);
      return true;
    } catch (e: any) {
      setBuildError(e?.message || "Puter fallback failed");
      return false;
    }
  };
  ...
  const confirmBuild = async () => {
    ...
    if (attachedFiles.length === 0) {
      ...
      if (!launched.ok || !launched.data) {
        // Hybrid fallback: if Totalum says no key / auth error, try Puter
        const isAuthError = launched.code === "UNKNOWN" || launched.error?.toLowerCase().includes("api key") || launched.error?.toLowerCase().includes("unauthorized") || configMode === "puter";
        if (isAuthError && typeof window !== "undefined") {
          const ok = await createViaPuterFallback(id, firstPrompt.trim());
    ...
    if (!res.ok) {
      const isAuthError = res.code === "UNKNOWN" || res.error?.toLowerCase().includes("api key") || configMode === "puter";
      if (isAuthError && typeof window !== "undefined") {
        const ok = await createViaPuterFallback(id, firstPrompt.trim());
        setBuildCreating(false);
        if (ok) return;
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <header className="sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-12 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Link href="/vs/bolt" className="hidden sm:inline-flex items-center gap-1.5 text-xs font-semibold bg-gray-900 text-white px-3 py-1.5 rounded-full hover:bg-black"><Crown className="w-3 h-3 text-amber-400"/> ขอบเขตการใช้งาน</Link>
            <Link href="/">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        {/* Bolt Killer Banner — ขอบเขตการใช้งาน ขาดๆ */}
        ...
        {!loading && (
          <div className={hasProjects || keyConfigured === false ? "mb-10" : "flex flex-col items-center justify-center min-h-[50vh]"}>
            <div className="w-full max-w-2xl mx-auto">
              ...
              {/* Setup guidance — hybrid: Totalum banner when unconfigured, Puter banner when in Puter mode */}
              {keyConfigured === false && <SetupBanners />}
              {configMode === "puter" && !hasProjects && <PuterSetupBanner />}
              {configMode === "puter" && hasProjects && (
                <div className="mt-4 flex items-center gap-2 text-xs text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2">
                  <PuterStatusBadge />
                  <span>— Hybrid Mode: projects stored via Puter, deploy to puter.site</span>
                </div>
              )}
            </div>
          </div>
