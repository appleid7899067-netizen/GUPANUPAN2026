...
export default function DashboardPage() {
  ...
  */
  ...
  const createViaPuterFallback = async (id: string, prompt: string) => {
    try {
      const { puterCreateProject } = await import("@/lib/puter");
      const proj = await puterCreateProject({ projectId: id, description: prompt.slice(0, 200), label: id });
      try { sessionStorage.setItem(`vibebuild:pendingPrompt:${proj.projectId}`, prompt); } catch {}
      toast.success(`Created via Puter: ${proj.projectId}`);
      router.push(`/project/${proj.projectId}`);
      return true;
    } catch (e: any) {
